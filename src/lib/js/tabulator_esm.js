...
class Popup extends CoreFeature {
	...
	_fitToScreen(x, y, parentEl, parentOffset, position) {
		...
		if (
			...
			Math.max(this.container.offsetHeight, scrollTop ? this.container.scrollHeight : 0)
		) {
			if (parentEl) {
				switch (position) {
					case 'bottom':
				...
				if (accessor) {
					...
					if (value != 'undefined') {
						...
						column.setFieldValue(
							data,
							accessor.accessor(value, data, type, params, colComponent, rowComponent)
						);
					}
				}
			}
	...
	initializeTopRow() {
		if (!this.topInitialized) {
			this.table.columnManager
				.getContentsElement()
				.insertBefore(this.topElement, this.table.columnManager.headersElement.nextSibling);
...
class DataTree extends Module {
	...
	initializeElementField() {
		...
		this.elementField =
			this.table.options.dataTreeElementColumn || (firstCol ? firstCol.field : false);
	}
...
function input(cell, onRendered, success, cancel, editorParams) {
	...
	function onChange(e) {
		if (
			((cellValue === null || typeof cellValue === 'undefined') && input.value !== '') ||
...
function date(cell, onRendered, success, cancel, editorParams) {
	...
	function onChange() {
		...
		if (
			((cellValue === null || typeof cellValue === 'undefined') && value !== '') ||
			value !== cellValue
		) {
...
class Edit {
	...
	_scrollToValue(char) {
		...
		var match = this.displayItems.find((item) => {
			return (
				typeof item.label !== 'undefined' && item.label.toLowerCase().startsWith(this.filterTerm)
			);
		});
		...
		for (let key in editorParams.elementAttributes) {
			if (key.charAt(0) == '+') {
				...
				starsHolder.setAttribute(
					key,
					starsHolder.getAttribute(key) + editorParams.elementAttributes['+' + key]
				);
			} else {
	...
	focusScrollAdjust(cell) {
		if (this.table.rowManager.getRenderMode() == 'virtual') {
			var topEdge = this.table.rowManager.element.scrollTop,
				bottomEdge =
					this.table.rowManager.element.clientHeight + this.table.rowManager.element.scrollTop,
			...
			var leftEdge = this.table.rowManager.element.scrollLeft,
				rightEdge =
					this.table.rowManager.element.clientWidth + this.table.rowManager.element.scrollLeft,
				cellEl = cell.getElement();
...
class Export extends Module {
	...
	processColumnGroup(column) {
		var subGroups = column.columns,
			...
			title =
				column.definition[
					'title' + (this.colVisProp.charAt(0).toUpperCase() + this.colVisProp.slice(1))
				] || column.definition.title;
		...
		headers.forEach((header) => {
			...
			header.forEach((col) => {
				if (col) {
					...
					columns.push(
						new ExportColumn(title, col.column.getComponent(), col.width, col.height, col.depth)
					);
				} else {
					columns.push(null);
				}
	...
	generateTableElement(list) {
		var table = document.createElement('table'),
			...
			rowFormatter =
				this.table.options[
					'rowFormatter' + (this.colVisProp.charAt(0).toUpperCase() + this.colVisProp.slice(1))
				],
			setup = {};
		...
		setup.groupHeader =
			this.table.options[
				'groupHeader' + (this.colVisProp.charAt(0).toUpperCase() + this.colVisProp.slice(1))
			];
		...
		row.columns.forEach((column) => {
			if (column) {
				...
				if (column.component._column.parent) {
					this.mapElementStyles(column.component._column.parent.groupElement, cellEl, [
						'border-top'
					]);
				}
...
function tickCross$1(cell, formatterParams, onRendered) {
	...
	if (
		...
		if (
			empty &&
			(value === 'null' || value === '' || value === null || typeof value === 'undefined')
		) {
...
class Menu extends Module {
	...
	initializeColumn(column) {
		...
		if (def.headerContextMenu && !this.columnSubscribers.headerContextMenu) {
			this.columnSubscribers.headerContextMenu = this.loadMenuTableColumnEvent.bind(
				this,
				'headerContextMenu'
			);
		...
		if (def.headerClickMenu && !this.columnSubscribers.headerClickMenu) {
			this.columnSubscribers.headerClickMenu = this.loadMenuTableColumnEvent.bind(
				this,
				'headerClickMenu'
			);
		...
		if (def.headerDblClickMenu && !this.columnSubscribers.headerDblClickMenu) {
			this.columnSubscribers.headerDblClickMenu = this.loadMenuTableColumnEvent.bind(
				this,
				'headerDblClickMenu'
			);
					...
					if (diff > 0) {
						...
						if (prevCol && -diff - prevColWidthLast > prevColWidth) {
							...
							if (moveToCol !== column) {
								...
								moveToCol
									.getElement()
									.parentNode.insertBefore(this.placeholderElement, moveToCol.getElement());
...
class MoveRows extends Module {
	...
	initializeRow(row) {
		...
		config.mousemove = function (e) {
			...
			if (
				e.pageY - Helpers.elOffset(rowEl).top + self.table.rowManager.element.scrollTop >
				row.getHeight() / 2
			) {
				...
				if (diff > 0) {
					...
					if (prevRow && -diff - prevRowHeightLast > prevRowHeight) {
						...
						if (moveToRow !== row) {
							...
							moveToRow
								.getElement()
								.parentNode.insertBefore(this.placeholderElement, moveToRow.getElement());
...
class Mutator extends Module {
	...
	transformCell(cell, value) {
		if (cell.column.modules.mutate) {
			var mutator =
					cell.column.modules.mutate.mutatorEdit || cell.column.modules.mutate.mutator || false,
				tempData = {};
...
function rows(pageSize, currentRow, currentPage, totalRows, totalPages) {
	...
	if (totalRows) {
		valueEl.innerHTML =
			' ' + currentRow + '-' + Math.min(currentRow + pageSize - 1, totalRows) + ' ';
...
var defaultWriters = {
	...
	cookie: function (id, type, data) {
		...
		document.cookie =
			id + '-' + type + '=' + JSON.stringify(data) + '; expires=' + expireDate.toUTCString();
	}
};
...
class Popup$1 extends Module {
	...
	initializeColumn(column) {
		...
		if (def.headerContextPopup && !this.columnSubscribers.headerContextPopup) {
			this.columnSubscribers.headerContextPopup = this.loadPopupTableColumnEvent.bind(
				this,
				'headerContextPopup'
			);
		...
		if (def.headerClickPopup && !this.columnSubscribers.headerClickPopup) {
			this.columnSubscribers.headerClickPopup = this.loadPopupTableColumnEvent.bind(
				this,
				'headerClickPopup'
			);
		...
		if (def.headerDblClickPopup && !this.columnSubscribers.headerDblClickPopup) {
			this.columnSubscribers.headerDblClickPopup = this.loadPopupTableColumnEvent.bind(
				this,
				'headerDblClickPopup'
			);
...
class ResizeColumns extends Module {
	...
	initializeColumn(type, component, column, element) {
		...
		if (type === 'header') {
			variableHeight =
				column.definition.formatter == 'textarea' || column.definition.variableHeight;
...
class ResizeRows extends Module {
	...
	_mouseDown(e, row, handle) {
		...
		function mouseMove(e) {
			row.setHeight(
				self.startHeight +
					((typeof e.screenY === 'undefined' ? e.touches[0].screenY : e.screenY) - self.startY)
			);
		}
...
class ResponsiveLayout extends Module {
	...
	initializeResponsivity() {
		...
		this.collapseFormatter =
			this.table.options.responsiveLayoutCollapseFormatter || this.formatCollapsedData;
...
class Sort extends Module {
	...
	initializeColumn(column) {
		...
		if (column.definition.headerSort !== false) {
			...
			//sort on click
			(this.table.options.headerSortClickElement === 'icon' ? arrowEl : colEl).addEventListener(
				'click',
				(e) => {
					var dir = '',
						sorters = [],
				...
				switch (position) {
					...
					case 'center':
						if (
							...
							this.elementVertical.scrollTop =
								this.elementVertical.scrollTop - this.elementVertical.clientHeight / 2;
						}
					...
					this.getVisibleRows().forEach((row) => {
						if (row.type !== 'group') {
							...
							row
								...
								.insertBefore(
									cell.getElement(),
									row.getCell(this.columns[this.rightCol]).getElement().nextSibling
								);
							cell.cellRendered();
						}
					});
...
class ColumnManager extends CoreFeature {
		...
		return new Promise((resolve, reject) => {
			...
			if (column.visible) {
				...
				left = Math.max(
					Math.min(
						left,
						this.table.rowManager.element.scrollWidth - this.table.rowManager.element.clientWidth
					),
					0
				);
	...
	getFlexBaseWidth() {
		...
		if (this.table.rowManager.element.scrollHeight > this.table.rowManager.element.clientHeight) {
			totalWidth -=
				this.table.rowManager.element.offsetWidth - this.table.rowManager.element.clientWidth;
		}
...
class VirtualDomVertical extends Renderer {
	...
	scrollRows(top, dir) {
		...
		if (-topDiff > margin || bottomDiff > margin) {
			...
			this._virtualRenderFill(
				Math.floor(
					(this.elementVertical.scrollTop / this.elementVertical.scrollHeight) * rows.length
				)
			);
			this.scrollColumns(left);
		} else {
	...
	resize() {
		this.vDomWindowBuffer =
			this.table.options.renderVerticalBuffer || this.elementVertical.clientHeight;
	}
		...
		if (!this.table.destroyed) {
			...
			if (this.redrawBlock) {
				...
				if (Helpers.elVisible(this.element)) {
					if (renderInPosition) {
						this.reRenderInPosition(
							this.refreshPipelines.bind(this, handler, stage, index, renderInPosition)
						);
					} else {
